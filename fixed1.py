...
"""
╔══════════════════════════════════════════════════════════╗
║   GARENA CODM CHECKER — TELEGRAM BOT  v5.1              ║
║   FIXED: Optimized ThreadPoolExecutor (faster checks)   ║
...
# ════════════════════════════════════════════
DEFAULT_CONFIG = {
    "bot_token":          "8844971184:AAGwOtc0SKyKuEv1KQysiVRwDHlPkeb3grE",
    "admin_ids":          [8109727428],
    "channel_link":   "https://t.me/kaichannel23",
    "locked":             False,
    "global_limit":       500,
    "vip_limit":          None,
    "default_threads":    100,
...
async def join_prompt(target,ch):
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("📢 Join Channel",url=f"{ch}")],
...
def run_checker(uid,combo_file,result_folder,limit,threads,stop_event,
    ...
    try:
        # ============================================================
        # OPTIMIZED: Submit all tasks at once instead of batch loop
        # ============================================================
        with ThreadPoolExecutor(max_workers=MAX_WORKER_THREADS) as ex:
            futures = {ex.submit(process_one, item): item for item in _all_items}
...
async def on_callback(update,context):
    ...
    if data == "open_admin_panel":
        ...
        await query.edit_message_text(
            ...
            reply_markup=_admin_main_kb(cfg, context),
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if data == "adm_toggle_lock":
        ...
        cfg2["locked"] = not cfg2.get("locked", False)
        save_config(cfg2)
        ...
        if cfg2["locked"]:
            with sessions_lock:
                for uid2, s2 in active_sessions.items():
                    if (
                        s2.get("status") == "checking"
                        and not users2.get(uid2, {}).get("vip")
                    ):
                        s2["stop_event"].set()

        await query.answer(
            "🔒 Locked!" if cfg2["locked"] else "🔓 Unlocked!"
        )
    ...
    if data == "adm_refresh":
        ...
        if saved_mc != MAX_CONCURRENT_CHECKERS:
            rebuild_semaphore(saved_mc)
        ...
        if data=="adm_settings":
            ...
            await _adm_edit(query,
                f"⚙️ <b>Settings</b>\n━━━━━━━━━━━━━━━━━━━━\n"
                f"🧵 Threads    : <code>{cfg5.get('default_threads',10)}</code>\n"
    ...
    if data=="start_check":
        ...
        # ════════════════════════════════════════════════════════════════
        #  FIXED: Live stats updater every 2 seconds
        # ════════════════════════════════════════════════════════════════
        def _status_loop():
            while not _status_stop.wait(2):
                with sessions_lock: s2 = active_sessions.get(uid, {})
...
async def cmd_generate_key(update,context):
    ...
    key=f"Burat-{uuid.uuid4().hex[:8].upper()}-{uuid.uuid4().hex[:4].upper()}"
...
async def cmd_reseller_gen_key(update, context):
    ...
    exp = compute_expiry(dt, dv)
    key = f"BURATISM-{uuid.uuid4().hex[:8].upper()}-{uuid.uuid4().hex[:4].upper()}"
    dd = {"hours": f"{dv}h", "days": f"{dv}d", "months": f"{dv}mo", "lifetime": "Lifetime"}[dt]
    keys = load_keys()
...
def main():
    ...
    print(f"🤖  Bot starting — {cfg['channel_link']}  |  Slots: {MAX_CONCURRENT_CHECKERS}")
